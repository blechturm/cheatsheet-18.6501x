...
$f_ p(y) =\\  =\exp (y \underbrace{(\ln (p)-\ln (1-p))}_{\theta } + \underbrace{n\ln (1-p)}_{-b(\theta )} +\underbrace{\ln(\binom {n}{y})}_{c(y,\phi )} )$
...
Parameters $\lambda, \theta \in \mathbb{R}$, continuous\\
$ f_x(x)=
	\begin{cases}
		 \lambda exp(-\lambda(x - \theta )),&{x >= \theta}\\
		0,&{x <= \theta}\\
	\end{cases}
$\\
...
$L(X_1\dots X_n;\lambda,\theta)= \lambda^n \exp(-\lambda n (\bar{X}_n - \theta)) 1(X_1 \ge \theta)$
